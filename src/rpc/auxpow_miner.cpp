...
void auxMiningCheck(const JSONRPCRequest& request)
{
  ...
  bool testChain = Params().MineBlocksOnDemand() || Params().NetworkIDString() != CBaseChainParams::MAIN;

  if (!testChain && node.connman->GetNodeCount (CConnman::CONNECTIONS_ALL) == 0)
      throw JSONRPCError (RPC_CLIENT_NOT_CONNECTED, "Cyberyen is not connected!");

  if (!testChain && ::ChainstateActive ().IsInitialBlockDownload ())
      throw JSONRPCError (RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Cyberyen is downloading blocks...");
