...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    static auto is_cy = [](const CInputCoin& input) { return !input.IsMWEB(); };
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_cy)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple CY-to-CY transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::CY_TO_CY;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::CY_TO_CY) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    if (cy_input_amount > 0) {
        const CAmount cy_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(cy_fee <= cy_input_amount);
        pegin_amount = (cy_input_amount - (cy_fee + cy_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, cy_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& cy_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + cy_change);
    if (change_amount < 0) {
