Cyberyen Core version 0.13.2 is now available from:

  <https://download.cyberyen.org/cyberyen-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a cyberyen
...
ECDSA signatures inside Cyberyen transactions now use validation using
...
Previous versions of Cyberyen Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Cyberyen Core was to raise the default minimum
relay fee.

Cyberyen Core 0.13.2 will have a strict maximum size on the mempool. The
...
Cyberyen Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running cyberyend without having to do any manual configuration.
...
Cyberyen Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Cyberyen Core will relay transactions with
...
entirely in Cyberyen Core 0.13, and it is at this time undecided whether the
...
Cyberyen Core automatically creates a hidden service to listen on, without
manual configuration. Cyberyen Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Cyberyen Core is listening, and
...
Cyberyend can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Cyberyen
Core to use floating fees. By default, Cyberyen Core will use floating
fees.
...
(default: `0.0002` CY/kB).

At all times, Cyberyen Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CY.
Furthermore, Cyberyen Core will never create transactions paying less than
...
`cyberyen.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /cyberyenrpc>
        ...
        # optional bypass cyberyend rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< cyberyenrpc:<password>
    </Location>
...
Restarting cyberyend will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in cyberyen.conf
...
cyberyen-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/cyberyen-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Cyberyen Core code base has
...
- `cyberyen-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `cyberyen-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Cyberyen scripting system that in combination with
...
Cyberyen transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Cyberyen Core.
...
The two phases are distinct in the Cyberyen-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cyberyen wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Cyberyen.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cyberyen block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of cyberyens they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of cyberyens being spent is exactly
...
- **Increased security for multisig:** Cyberyen addresses (both P2PKH addresses
  ...
  believed to be provided by Cyberyen's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Cyberyen's current ledger (called the UTXO set),
  ...
  note that Cyberyen Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cyberyen users to individually opt-in to almost any change in the Cyberyen
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Cyberyen Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Cyberyen
client to Cyberyen Core 0.13.2 to signal for segwit support. When a
...
Since Cyberyen Core 0.10.0, nodes have defaulted to only relaying and
...
- `cyberyen-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `cyberyen-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
