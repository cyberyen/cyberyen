...
The Cyberyen Core daemon can be configured to act as a trusted "border
router", implementing the cyberyen wire protocol and relay, making
...
The ZeroMQ feature in Cyberyen Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cyberyend:
...
For instance:
    ...
    $ cyberyend -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/cyberyend.tx.raw \
...
These options can also be provided in cyberyen.conf.
...
From the perspective of cyberyend, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cyberyend directly. Furthermore, no information is
...
using. Cyberyend appends an up-counting sequence number to each
