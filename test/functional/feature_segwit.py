...
from test_framework.blocktools import subsidy_cy, witness_script, send_to_witness
...
class SegWitTest(BitcoinTestFramework):
    ...
    def success_mine(self, node, txid, sign, redeem_script=""):
        send_to_witness(1, node, getutxo(txid), self.pubkey[0], False, subsidy_cy() - Decimal("0.002"), sign, redeem_script)
    ...
    def fail_accept(self, node, error_msg, txid, sign, redeem_script=""):
        assert_raises_rpc_error(-26, error_msg, send_to_witness, use_p2wsh=1, node=node, utxo=getutxo(txid), pubkey=self.pubkey[0], encode_p2sh=False, amount=subsidy_cy() - Decimal("0.002"), sign=sign, insert_redeem_script=redeem_script)
    ...
    def run_test(self):
        ...
        for _ in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], subsidy_cy()), self.pubkey[n], False, subsidy_cy() - Decimal("0.001")))
                    p2sh_ids[n][v].append(send_to_witness(v, self.nodes[0], find_spendable_utxo(self.nodes[0], subsidy_cy()), self.pubkey[n], True, subsidy_cy() - Decimal("0.001")))

        self.nodes[0].generate(1)  # block 163
        self.sync_blocks()

        # Make sure all nodes recognize the transactions as theirs
        assert_equal(self.nodes[0].getbalance(), balance_presetup - 60 * subsidy_cy() + 20 * (subsidy_cy() - Decimal("0.001")) + subsidy_cy())
        assert_equal(self.nodes[1].getbalance(), 20 * (subsidy_cy() - Decimal("0.001")))
        assert_equal(self.nodes[2].getbalance(), 20 * (subsidy_cy() - Decimal("0.001")))
        ...
        txid1 = send_to_witness(1, self.nodes[0], find_spendable_utxo(self.nodes[0], subsidy_cy()), self.pubkey[0], False, subsidy_cy() - Decimal("0.004"))
        ...
        tx.vout.append(CTxOut(int((subsidy_cy() - Decimal("0.01")) * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))
        ...
        tx.vout.append(CTxOut(int((subsidy_cy() - Decimal("0.05")) * COIN), CScript([OP_TRUE, OP_DROP] * 15 + [OP_TRUE])))  # Huge fee
        tx.calc_sha256()
    ...
    def mine_and_test_listunspent(self, script_list, ismine):
        utxo = find_spendable_utxo(self.nodes[0], subsidy_cy())
        tx = CTransaction()
